...
impl LcgStream {
    ...
    fn peek_bytes(&mut self, count: usize) -> Vec<u8> {
        let mut temp_lcg = Lcg { state: self.lcg.state };
        ...
        loop {
            match stream_clone.read(&mut buf) {
                ...
                Ok(n) => {
                    ...
                    println!("Key: {} (keystream position: {})", format_hex_bytes(&key_bytes), start_pos);
                    
                    let plain_str = String::from_utf8_lossy(&plain);
                    println!("Plain: {} -> \"{}\"", format_hex_bytes(&plain), plain_str.trim());
                    
                    let re_cipher: Vec<u8> = plain.iter().zip(key_bytes.iter()).map(|(p, k)| p ^ k).collect();
                    if re_cipher == cipher {
